...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    ///`StoreConnector` :: to gain access to our `store` anywhere in our application
...
class ListInputState extends State<ListInput> {
  ...
  Widget build(BuildContext context) {
    ...
    ///With `StoreConnector` :: connects our current widget with the `store` of the applicaiton
    ...
    ///     that we can pass into the second property
    ///
    /// `#2 builder`
    ///
    ...
    //       ),
    //   builder: (context, _ViewModel viewModel) {   //********* check the second Parameter  */
    //     return TextField(
    ...
    return StoreConnector<ListState, VoidCallback>(
        ...
        builder: (context, VoidCallback dispatch) => TextField(
              controller: controller,
              decoration: InputDecoration(hintText: "Enter an item"),
              onSubmitted: (text) {
                dispatch();
